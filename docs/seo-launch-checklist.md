...
- The MinistryPlatform user-login widget passed a live whitelist test on the GitHub Pages review origin on July 22, 2026. The approved event finder, event details, and event checkout widgets are implemented on the public Events page and the two hidden utility pages. This is the complete approved widget set. Giving remains linked to St. Ann's Realm page, while prayer/support and parish updates use the site's existing contact, bulletin, and social-media paths.
